class CouplesDeriver():
    ...
    def derive(self, i=0, derived_matches=set(), derived_no_matches=set()):
        ...
        unknown_couples = set([couple for couple in guess if self.couple_not_in_confirmed_or_derived_lists(couple, derived_matches, derived_no_matches)])
        print("unknown couples are {0}".format(unknown_couples))
    ...
    def couple_not_in_confirmed_or_derived_lists(self, couple, derived_matches, derived_no_matches):
        return not couple_in_list(self.confirmed_matches, couple) and \
               not couple_in_list(self.confirmed_no_matches, couple) and \
               not couple_in_list(derived_no_matches, couple) and \
               not couple_in_list(derived_matches, couple)

    def derive_no_matches(self, guess, no_correct, unknown_couples):
        unknown_couples_are_no_matches = number_of_couples_in_guess_from_list(guess,
                                                                              self.confirmed_matches) == no_correct
    ...
    def derive_matches(self, guess, no_correct, unknown_couples):
        unknown_couples_are_matches = len(guess) - number_of_couples_in_guess_from_list(guess,
                                                                                        self.confirmed_no_matches) == no_correct
